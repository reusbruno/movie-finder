...
- Vibe blending's score weights (genre 1/2, keyword 2/4, recommendation 2/6 - see `src/lib/vibe-blend.ts`) are a reasoned starting point, not tuned against real usage. Revisit once there's a sense of whether results skew too genre-heavy, too obscure (candidate pool is capped to one discover page / 20 items, popularity-sorted before scoring), or too `/recommendations`-dominated. Also worth watching: `blendTitles` fires one TMDB keyword call per candidate (~20 per blend, parallelized, best-effort) plus `enrichMoviesWithRatings` fires ratings lookups for the full result set at once - under OMDb's already-exhausted free-tier quota this surfaced as `ETIMEDOUT`/`fetch failed` in testing (degrades to no ratings, doesn't break the blend, but is a burst of concurrent calls worth knowing about if OMDb rate limits get stricter).

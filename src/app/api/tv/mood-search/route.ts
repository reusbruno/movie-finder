...
// bucket per route, only enforced on the fresh-query (LLM) path.
const RATE_LIMIT = 10;
...
export async function POST(request: NextRequest) {
  ...
  if (sortByOverride !== undefined) {
    if (!(TV_SORT_OPTIONS as readonly string[]).includes(sortByOverride as string)) {
      return NextResponse.json(
        { error: `Field 'sortBy' must be one of: ${TV_SORT_OPTIONS.join(", ")}` },
        { status: 400 }
      );
    }
  }
  ...
  try {
    const { genres } = await getTVGenres(tmdbLanguage);
    ...
    const mergedSortBy = (TV_SORT_OPTIONS as readonly string[]).includes(merged.sortBy)
    ...
    // user-overridden, fall back to the original LLM extraction for the
    // chips rather than showing the user's own checkbox pick back to them.
